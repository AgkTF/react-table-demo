...
export function TractsTableByDS() {
  ...
  const sortingMiddleware = useSortingMiddleware<Tract>();
  const resizingMiddleware = useResizingMiddleware<Tract>({
  ...
  const toggleColsVisibility = useCallback(
    ...
    [setColumnVisibility, table.getAllLeafColumns()]
  );
